...
Current parity baseline after iteration 136:

- `basic-auth-context-client` remains intentionally thin, but the reference app now consumes its root/browser helper story plus `@securitydept/basic-auth-context-client-react` for login entry wiring and playground ownership
- `session-context-client` now owns the reference app's session login redirect, pending redirect, user-info, and logout story through `@securitydept/session-context-client-react`
- the remaining asymmetry is intentional: neither family duplicates token-set callback orchestration, token material persistence, or bearer transport ownership

Iteration 137 follow-up:

- the remaining framework-neutral session convenience has now been promoted from React adapter glue into `SessionContextClient` core so React and Angular consume one canonical browser-shell story
- the `basic-auth-context` family audit found no equivalent React-only convenience leak; its shared boundary helpers already live in core and the adapters remain intentionally thin host wrappers

Iteration 142 follow-up:

- the shared authenticated-principal baseline is now explicit across TypeScript, webui host projection, and Rust auth crates: `@securitydept/client` owns the TS/browser semantic baseline and `securitydept-utils::principal` owns the Rust/server baseline
- session-context and token-set no longer maintain divergent human-principal semantic owners; any remaining separate structs on the backend side are wire/transport DTOs, not second semantic owners
- resource-token facts remain intentionally separate from authenticated human-principal projection, and mixed-custody / BFF / server-side token ownership still remain later-scope topics rather than implied by this consolidation
...
- the richer source-helper baseline is now productized across foundation and web/browser layers (`fromSignal`, `fromPromise`, `fromAbortSignal`, `fromStorageEvent`), but the broader operator family and heavier stream-composition DSLs remain future work
- capability-first configuration layering is now a real adopter-facing baseline rather than only a direction: runtime/foundation config, auth-context config, and adapter/host config are now the formal three layers; `frontend-oidc-mode` owns browser materialization via `createFrontendOidcModeBrowserClient()`, and the remaining non-baseline is the absence of one flattened global config DSL
- logger / trace sink / operation tracer / testing observation hierarchy are no longer only design discipline: iteration 121 productized the minimal shared trace path (`TraceEvent`, `createTraceTimelineStore()`, `FrontendOidcModeTraceEventType`, reference-app trace timeline consumption, and direct trace assertions). Iteration 135 then completed the first cross-runtime consolidation pass: token-set frontend and backend now share one explicit structured-trace host story, Basic Auth and browser harness now sit on a formal observation hierarchy. Iteration 141 then productized the missing TS operation-correlation layer itself through `createOperationTracer()`, `OperationTraceEventType`, operation-aware `InMemoryTraceCollector` support, token-set callback/refresh correlation, and reference-app timeline consumption. Heavier exporter / cross-service ingestion / full OTel work still remains future scope
...
- Rust / server-side structured observability is no longer only a future topic: iteration 123 established the minimal shared auth-flow diagnosis baseline in `securitydept-utils::observability`, adopted it on projection/config fetch, callback/token refresh, forward-auth, and propagation, and proved at least one direct machine-readable test-consumption path. Iteration 135 extended that same owner to session-context login/logout/user-info and basic-auth login/logout/authorize, with `apps/server` routes and middleware now consuming those diagnosed results directly. Iteration 143 then hardened the vocabulary and route evidence: `AuthFlowOperation` / `AuthFlowDiagnosisField` now own the shared Rust diagnosis taxonomy, `apps/server` uses a thin route diagnosis logging adapter, dashboard auth boundary branches now emit machine-readable diagnosis, and credential-management groups/entries routes now emit secret-safe route diagnosis. Broader exporter/timeline work and still-wider route coverage remain future topics
...
2. **Validation abstraction and input/runtime completeness**
   ...
   - ~~a minimal unified input-source / scheduling baseline above the raw scheduler abstraction~~ (implemented and later consolidated into the richer current baseline: `timer`, `interval`, `scheduleAt`, `fromEventPattern`, `fromSignal`, and `fromPromise` in `@securitydept/client`; `fromVisibilityChange`, `fromAbortSignal`, and `fromStorageEvent` in `@securitydept/client/web`; real adoption now includes React signal bridging, callback-resume promise settlement, cross-tab sync, and browser cancellation interop)
...
7. **Shared cancellation / resource-release baseline**
   - ~~shared TS/browser cancellation contract plus browser interop~~ (implemented in iteration 138: `@securitydept/client` now formalizes `DisposableTrait`, `CancellationTokenTrait`, `CancellationTokenSourceTrait`, `createCancellationTokenSource()`, and `createLinkedCancellationToken()` as the current cooperative cancellation/resource-release baseline; `@securitydept/client/web` now owns both `createAbortSignalBridge()` and `createCancellationTokenFromAbortSignal()` as the canonical browser interop pair)
   - ~~reference app should stop owning app-local AbortSignal bridge glue~~ (implemented: `apps/webui/src/api/tokenSet.ts` now consumes the shared web helper instead of hand-rolling `AbortSignal -> CancellationTokenTrait`)
   - remaining non-baseline: `Symbol.dispose` interop, linked cancellation source trees beyond token linking, and broader ambient/global teardown registries

8. **Capability-first configuration layering baseline**
   - ~~formalize the current three-layer configuration story~~ (implemented in iteration 139: runtime/foundation config, auth-context config, and adapter/host config are now written as the canonical configuration layering across `007`, inventory, and adapter/provider entry docs)
   - ~~reference app should stop owning frontend-mode config/materialization assembly~~ (implemented: `@securitydept/token-set-context-client/frontend-oidc-mode` now owns `createFrontendOidcModeBrowserClient()`, `resolveFrontendOidcModePersistentStateKey()`, and `resolveFrontendOidcModeBrowserStorageKey()`; `apps/webui/src/lib/tokenSetFrontendModeClient.ts` now keeps only host bootstrap, host trace, and host-specific cross-tab reconciliation)
   - ~~adapter/provider options should stop reading like one flat config blob~~ (implemented: Basic Auth / Session / token-set React and Angular entry surfaces now document the same split between auth-context config, runtime capabilities, and host registration glue)
   - remaining non-baseline: no single global config DSL across all auth families, no Rust server config rewrite, and no ambient cross-family config registry
...
- continue extending the Rust / server auth-flow diagnosis baseline beyond the current productized operations (`projection.config_fetch`, `oidc.callback`, `oidc.token_refresh`, `forward_auth.check`, `propagation.forward`, session/basic-auth auth-context operations, `dashboard_auth.check`, and the `creds_manage.group.*` / `creds_manage.entry.*` route operations) while keeping `securitydept-utils::observability` as the shared vocabulary owner and leaving broader exporter/timeline work outside the current baseline

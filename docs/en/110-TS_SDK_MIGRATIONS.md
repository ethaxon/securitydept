...
- `FoundationEnvironment.realmStorage` is now a required volatile `SyncStorageTrait`. General `StorageTrait` operations may return direct values or Promises, and storage adapters may expose logical-key changes through `storageEvent`. `createFoundationEnvironment()` and all derived host factories create a fresh isolated synchronous in-memory realm store by default; composition roots may override it through options or the `REALM_STORAGE_TRAIT_TOKEN` provider. Manually constructed environment objects must add this field.

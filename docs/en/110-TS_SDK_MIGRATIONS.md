...
`TraceTimelineStore` no longer requires the `mnemonist` peer. Remove a direct `mnemonist` dependency if it was installed only for the timeline subscriber. The store now uses an SDK-internal fixed-capacity ring buffer while preserving its public behavior: subscribe to `latestEntry: EventStreamTrait<TraceTimelineEntry | null>` for edge notifications, where `null` means the timeline was cleared, and read the current readonly array snapshot from the `entries` getter. The store does not publish a new full-array signal value for every recorded entry.

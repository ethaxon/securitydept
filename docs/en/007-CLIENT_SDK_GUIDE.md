...
- Page-only helpers may read `window.location` / `window.history` only through `NativeWebEnvironment`; return URL construction should stay explicit at the app boundary, for example `client.authorizeUrl(environment.router.currentUrl()?.toString())` or `client.loginWithRedirect({ postAuthRedirectUri })`. Backend OIDC callback pages should use `takeCompatFragmentFromRouter(router)` and then pass the returned compat payload directly to `client.handleCallback(fragment)`. Backend OIDC fragment redirects use the securitydept compat fragment protocol, so hash-router paths in `postAuthRedirectUri` are preserved. For token-set OIDC login, the shared browser entries are `loginWithRedirect({ postAuthRedirectUri })` and `loginWithPopup({ popupCallbackUrl })` on `BaseOidcModeClient`; the client itself must carry its page router / popup capability through its environment. `FrontendOidcModeClient` and `BackendOidcModeClient` implement those methods directly. `relayTokenSetPopupCallbackFromEnvironment()` remains a page-only helper and requires an explicit `environment` argument.
- Backend OIDC does not maintain a hidden callback-fragment store. Retry or delayed callback processing is an application-level policy: keep the fragment in the URL until processing succeeds, or persist it explicitly in application code. Helpers must fail fast when a required capability is missing instead of falling through to `window is not defined` or stale URL parsing.
...
- `provideClientEnvironment(environment)` and the planner-host factories (`AUTH_PLANNER_HOST`, `provideAuthPlannerHost()`, `AUTH_REQUIREMENTS_CLIENT_SET`, `provideRequirementsClientSet()`) register shared dependencies into the injector. They do not introduce additional domain-specific Providers or Context hooks.
...
- `provideSessionContext({ config, environment, initialRefresh })`: adapter leaf over `SessionContextController`; Angular DI reads `environment.transport` and `environment.sessionStorage` from the same `FoundationEnvironment` and registers the controller. Service construction does not auto-probe user-info unless `initialRefresh` is explicit.
...
- `provideNativeWebEnvironment({ environment })`: canonical Angular DI bridge for native-web-scoped capability injection. The canonical value is the host-owned `NativeWebEnvironment` object.

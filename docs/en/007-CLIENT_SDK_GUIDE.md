...
React is allowed exactly one SDK Context, all in `@securitydept/client-react`: `SecuritydeptContext`, `SecuritydeptProvider`, and `useSecuritydeptContext()`. Domain React packages no longer create their own public Context/Provider/`useXxxContext()` surface. They export injection tokens, provider factories, and explicit callback/component bridges. React reads synchronous signals through `useSignal()`, Resource snapshots through `useResourceSnapshot()`, Suspense values through the environment-scoped `useSuspenseResourceValue()`, and other observable/event sources through `useInteropObservable()` and `useEventStream()`.

`ResourceTrait.whenValue()` and `whenResourceSnapshotValue()` return the stale value carried by an `error` snapshot by default; `loading-error` still rejects because no value exists. Pass `{ staleValueWhenError: false }` when the caller requires strict error propagation.

The computed overload of `useSuspenseResourceValue()` requires both `dependencies` and an explicit `queryKey`. Dependencies only control the React memoized computation; they are not serialized into cache identity. The query key is the complete semantic identity of the computation and must include every variable that changes its result. Callers may explicitly include `useId()` when component-position scoping is intentional, but the adapter does not add it automatically. Stable Resource and snapshot-signal inputs continue to derive identity from the snapshot signal, with an optional query-key suffix.
...
| Registry | multi-client registration/readiness/discrimination | UI policy or framework state | `TokenSetClientRegistry` |
...
- Standalone client construction uses `BackendOidcModeClient.fromEnvironmentConfig({ config, environment, ...options })` or `FrontendOidcModeClient.fromEnvironmentConfig({ config, environment, ...options })`, where `environment` comes from `createFoundationEnvironment(...)`, `createEnvironmentForNativeWeb(...)`, or another explicit host creator. Framework-neutral DI hosts may instead install `provideBackendOidcModeClient(...)` or `provideFrontendOidcModeClient(...)` and resolve the matching mode-owned client token. Constructors are protected so subclasses can extend implementation details without creating a competing public construction contract. Registry-managed clients continue to use `createBackendOidcModeClientFactory()` / `createFrontendOidcModeClientFactory()` because those factories additionally own registry metadata, startup, and initialization cancellation. Do not pass transport, time, persistent store, and session store independently to every helper.
...
React composition still follows the three-layer model: core auth-context providers, a Fiber-external injector, and React view hooks.

- `@securitydept/client-react` owns `createEnvironmentForReact()`, the environment-scoped `QueryStore`, the only SDK React Context (`SecuritydeptContext`, `SecuritydeptProvider`, and `useSecuritydeptContext()`), and the bridges `useSignal()`, `useResourceSnapshot()`, `useSuspenseResourceValue()`, `useInteropObservable()`, and `useEventStream()`.
...
- `@securitydept/basic-auth-context-client` owns `BASIC_AUTH_CONTEXT_CLIENT`, `BASIC_AUTH_CONTEXT_CLIENT_CONFIG`, `provideBasicAuthContext({ config })`, and client construction. `@securitydept/basic-auth-context-client-react` only adds `useBasicAuthContextClient()`.
- `@securitydept/session-context-client` owns `SESSION_CONTEXT_CLIENT`, `SESSION_CONTEXT_CLIENT_CONFIG`, `provideSessionContext({ config })`, and client construction. `@securitydept/session-context-client-react` only adds `useSessionContextClient()`; use `useResourceSnapshot(client.sessionResource)` for rendering.
- `@securitydept/token-set-context-client/registry` owns `provideTokenSetClientRegistry()`, `TOKEN_SET_CLIENT_REGISTRY`, registry construction, and lifecycle. `@securitydept/token-set-context-client-react` adds `useTokenSetClientRegistry()` and headless callback hooks. Read keyed state through `useResourceSnapshot(registry.clientResourceFor("main"))`, then read the resolved client's `authResource`.
- `@securitydept/token-set-context-client/backend-oidc-mode` and `@securitydept/token-set-context-client/frontend-oidc-mode` own their standalone client tokens and provider graphs. React-specific packages do not recreate those clients or their lifecycle.
...
- `provideBasicAuthContext({ config })`: installs the core provider graph into the shared environment injector and maps its client instance to Angular DI.
...
- `provideSessionContext({ config })`: maps the core `SessionContextClient` instance to Angular DI. Use `config.autoStart` when construction should immediately start the client.
- `provideTokenSetClientRegistry({ clients })`: installs the core registry provider graph and maps the same `TokenSetClientRegistry` instance to Angular DI; each client entry still owns auth-context config.

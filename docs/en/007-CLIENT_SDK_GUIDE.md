...
- Frontend callbacks use `useTokenSetFrontendCallback({ clientQuery })`; backend callbacks use `useTokenSetBackendCallback()`. The hooks perform non-consuming registry selection, initialize the selected record, and flatten registry readiness with the selected client's `callback` Resource. Callback input resolution and cleanup are owned by the client during `start()`, before persistence restore. Backend registry selection uses the compat-fragment `callback_routing_key`; the selected client then conditionally takes only its own fragment. Frontend selection matches `callbackPath`, while the client resolver takes sensitive OIDC query parameters only after ownership is established. Neither framework adapter calls `client.start()` directly; the registry entry factory owns client construction and startup. After callback determination succeeds, in-memory auth and callback Resources are committed independently of best-effort persistence synchronization; a storage write failure is traced but does not turn the login into a failure.
...
- `TokenSetFrontendCallbackComponent` and `TokenSetBackendCallbackComponent` bridge registry/client callback Resources into Angular signals. They initialize the selected record but do not parse callback input or own a second callback state machine.

...
- `tests.yml` 拥有仓库验证。它在 `main`、`release`、`v*.*.*` tag、指向 `main` 的 pull request 与 manual dispatch 上运行，并上传 `tests-workflow-report`，方便 release run 按源 SHA 审计。
- `release.yml` 是唯一 release/build/publish authority。它由成功的 `Tests` `workflow_run` 触发，也支持 manual dispatch 显式传 source ref/SHA 与 publish toggles。
...
- 所有被选择的 publish jobs 成功后，`release-tag` job 会为 `release` 分支 source 创建并推送 expected `vX.Y.Z[-alpha.N|-beta.N]` tag。Tag 触发的 release 仍作为审计/重跑入口保留，但在 release 分支路径中，tag 是发布结果与审计锚点。

...
| Adapter | 最强证据 | 当前缺口 |
|---|---|---|
| `token-set-context-client/backend-oidc-mode/web` | Focused lifecycle tests（覆盖 callback precedence / recovery、retained JSON body replacement / reset-to-empty transition、shared-store fresh-client restore/reset）、reference app dogfooding、最小入口示例 | 更大范围 browser lifecycle hardening（cross-tab sync 等） |
| `token-set-context-client/backend-oidc-mode/react` | 最小 React focused test、入口示例、StrictMode remount/disposal focused test、reconfigure dispose/subscription-isolation focused test | React 17 / concurrent mode 未验证；更广泛宿主矩阵仍未覆盖 |

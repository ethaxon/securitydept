...
2. **token-set 特定 OIDC browser adapter**
...
- **通用 token orchestration 层**
  ...
  - bearer header 注入辅助
  - refresh 失败恢复策略
- **token-set 特定 browser adapter 层**
  - callback fragment 解析
...
这里要额外澄清：上面这组"通用 orchestration 层 + token-set 特定 adapter 层"的拆分，只是当前已落地的**共享基础设施 / 特定流程**内部模块边界。对外形状已收口为 OIDC mode family（详见 [008-OIDC-MODE-FAMILY](008-OIDC-MODE-FAMILY.md)）。

#### OIDC Mode Family（替代旧"三驾马车"术语）

从 iteration 51 开始，主术语从"三驾马车"切换为统一的 **OIDC mode family**：

| 模式 | 谁运行 OIDC 流程 | TS SDK 子路径 | Rust crate |
|---|---|---|---|
| `frontend-oidc` | 前端 | `/oidc` | `securitydept-oidc-client`（仅配置） |
| `backend-oidc-pure` | 后端 | （未来） | `securitydept-oidc-client` |
| `backend-oidc-mediated` | 后端（经中介） | `/token-set`（遗留名） | `securitydept-oidc-client` + `securitydept-token-set-context` |

完整 mode family 设计见 [008-OIDC-MODE-FAMILY](008-OIDC-MODE-FAMILY.md)。

#### `frontend-oidc`：前端纯 OIDC 客户端

- 前端通过 `oauth4webapi`（官方基座）处理 authorize/callback/token-exchange
- 后端只负责返回 OIDC 配置规则，不直接运行 OIDC 流程
- `oidc-client-ts` 作为 comparison/reference case（`devDependency` only）

依赖策略：
- `oauth4webapi`：官方基座，`optional peerDependency` + `devDependency`；使用 `/oidc` 的 adopter 需安装
- `oidc-client-ts`：comparison case，`devDependency` only；不对 adopter 产生安装要求

#### `backend-oidc-mediated`：原 "token-set" 的真正含义

当前 `/token-set` 子路径实际实现的是 `backend-oidc-mediated` 模式：
...
`oidc-client` 与 `oauth-resource-server` 通过 `OidcSharedConfig`（`securitydept-oauth-provider`）共享 provider 连接性配置，详见 [008-OIDC-MODE-FAMILY](008-OIDC-MODE-FAMILY.md) §2-3。

#### 跨模式约束

- OIDC mode family 三个模式都属于 `token-set-context-client` 产品家族
...
- SDK 可以逐步演进出 **headless orchestration primitive / scheduler direction**
- `token-set-context-client` 或其未来上层编排能力可以承担 pending requirement / callback recovery 一类状态机问题
- chooser UI、router policy、产品级交互步骤仍应留在 adopter 自己的 app glue 中

当前状态要说清楚：

- 这是一个高价值的下游参考案例方向
- 它有助于指导 `token-set-context-client` 的后续设计
...
对 `token-set-context-client` 来说，这意味着后续的“后端 pure OIDC client”与“后端 sealed refresh + metadata redemption flow”也应继续建立在同一 orchestration / lifecycle 基座上，而不是另起一套平行核心。
...
| `@securitydept/token-set-context-client` | bridge ² | 仅保留向后兼容 re-export；不再是任何场景层的 canonical 入口 | Root backward-compatible bridge |
| `@securitydept/token-set-context-client/token-set` | `stable` ² | Callback / restore / refresh / persistence / traceSink | Token-set sealed flow **canonical 入口** |
| `@securitydept/token-set-context-client/token-set/web` | `provisional` | `location` / `history` / `fetch` / flow-state storage | Token-set browser adapter canonical 入口 |
| `@securitydept/token-set-context-client/token-set/react` | `provisional` | React runtime | Token-set React adapter canonical 入口 |
| `@securitydept/token-set-context-client/web` | bridge | 旧路径，向后兼容；新代码应改用 `./token-set/web` | Browser adapter compat bridge |
| `@securitydept/token-set-context-client/react` | bridge | 旧路径，向后兼容；新代码应改用 `./token-set/react` | React adapter compat bridge |
| `@securitydept/token-set-context-client/orchestration` | `provisional` ³ | 不感知 token-set sealed flow；协议无关 token snapshot / persistence / transport / `AuthMaterialController` 薄控制层 | Token orchestration **显式子路径入口**（仍在同包，非独立 npm 包；推荐的协议无关入口） |
| `@securitydept/token-set-context-client/oidc` | `experimental` ⁴ | 前端纯 OIDC client（`frontend-oidc` 模式）；基于 `oauth4webapi`，不引入 `oidc-client-ts`；normalize token material 给 `/orchestration` | Frontend pure OIDC **显式子路径入口** |
...
² 此 `stable` 覆盖 `/token-set` canonical subpath 的 browser-owned token-set v1 contract。Root entry 为 bridge 入口，不再承担稳定性语义。Mixed-custody / BFF / server-side token-set 不在承诺范围内。

³ orchestration 能力现在通过 `@securitydept/token-set-context-client/orchestration` 显式子路径对外暴露，根入口同时保留向后兼容的 re-export。两个入口暴露同一层内容，仍承载在同一 npm 包内，未独立成新包。外部 adopter 可使用 `AuthMaterialController`（薄控制层）及其 `applyDelta()` 外部驱动更新入口，或直接使用底层 helper（`bearerHeader`, `createAuthStatePersistence`, `createAuthorizedTransport` 等）。控制层仅承接 token material lifecycle，不提供 acquisition / redirect / refresh scheduling 能力。它本身不是完整场景层，而是 OIDC mode family 三种模式（`frontend-oidc`、`backend-oidc-pure`、`backend-oidc-mediated`）共享的基座。官方前端 OIDC 封装使用 `oauth4webapi`，`oidc-client-ts` 为 comparison case（详见 [008-OIDC-MODE-FAMILY](008-OIDC-MODE-FAMILY.md)）。稳定性仍属冻结进行中（`provisional`）：已公开、有显式入口，但不是完整 `stable` 承诺。

⁴ `/oidc` 子路径实现 `frontend-oidc` 模式，以 `oauth4webapi` 为基座。当前为 `experimental`：已有显式入口和 normalize 逻辑，但 runtime surface 尚未完整冻结。使用 `/oidc` 的 adopter 需安装 `oauth4webapi`（`optional peerDependency`）。
统一解释：
...
- **通用 token orchestration**（`src/orchestration/`）
  ...
  - 不要求知道 token 的来源是标准 OIDC、后端 OIDC 还是 token-set sealed flow
- **token-set 特定 browser adapter**
  - 管 callback fragment、sealed payload、metadata redemption、redirect recovery
  - 这一层才感知 token-set 的特定协议形状
...
| `oidc/types.ts` | `OidcClientConfig` / `OidcTokenResult` / `AuthorizeResult` — 前端 pure OIDC client 配置与协议词汇 |
| `oidc/client.ts` | `createOidcClient()` — 封装 oauth4webapi 的标准 browser OIDC Authorization Code + PKCE 流程 |
...
- v1 public surface（`TokenSetContextClient`, `./web`, `./react`）完全未被打断
- `@securitydept/token-set-context-client/orchestration` 已成为**显式子路径入口**，推荐协议无关场景优先从此进入
- 根入口继续 re-export 同一套 orchestration exports，向后兼容
...
- `TokenSetContextClient` 内部已进一步建立在控制层之上：
  ...
  - `createTokenSetAuthorizedTransport()` 内部委托到 `createAuthorizedTransport()`
- 但 `/orchestration` 不应再被继续单独抽象推演成最终前端 OIDC 方案；下一阶段应直接用 `oauth4webapi`、`oidc-client-ts` 与未来 `angular-auth-oidc-client` 三组现实案例来校准“前端 pure OIDC client”这一驾
- 当前规划中的官方 pure-client OIDC 路线，仍位于 `token-set-context-client` 内部；它会以封装 `oauth4webapi` 为基座，并与后端 pure OIDC client / 后端 sealed refresh + metadata redemption flow 共用同一 orchestration 基础设施
- 当前默认预期是沿同一包内的 subpath / additive surface 继续演进，而不是先把它拆成并列新包
- iteration 50 完成了结构性重排：token-set v1 surface 从 root 下沉为显式 `/token-set` subpath 家族
- 当前 public subpath 家族：
  - `/token-set` — sealed refresh + metadata redemption flow（`stable` v1）
  - `/token-set/web` — browser adapter（`provisional`，canonical 路径已重排，稳定性待专项 evidence 提升）
  - `/token-set/react` — React adapter（`provisional`，canonical 路径已重排，稳定性待专项 evidence 提升）
  - `/orchestration` — 共享 protocol-agnostic lifecycle 基座（`provisional`）
  - `/oidc` — 前端 pure OIDC client，封装 oauth4webapi（`experimental`）
- root (`.`) 已降级为 backward-compatible bridge，不再是任何场景层的 canonical 入口
- 依赖语义：
...
|---|---|---|
| Browser App / SPA token-set | 用 `@securitydept/token-set-context-client/token-set` canonical 入口；browser bootstrap / callback / storage 走 `./token-set/web` | timeline UI、propagation probe、`apps/webui/src/api/*` 是 SDK surface |
...
| Adapter | 最强证据 | 当前缺口 |
|---|---|---|
| `token-set-context-client/web` | Focused lifecycle tests（覆盖 callback precedence / recovery、retained-fragment replacement / reset-to-empty transition、shared-store fresh-client restore/reset）、reference app dogfooding、最小入口示例 | 更大范围 browser lifecycle hardening（cross-tab sync 等） |
| `token-set-context-client/react` | 最小 React focused test、入口示例、StrictMode remount/disposal focused test、reconfigure dispose/subscription-isolation focused test | React 17 / concurrent mode 未验证；更广泛宿主矩阵仍未覆盖 |

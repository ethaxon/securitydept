...
4. **真实 multi-requirement orchestration baseline**
   ...
   - 剩余差距：
     ...
     - 剩余 projection source 差距（后续迭代）：
      ...
      - ~~React adapter 的 async readiness 等价机制~~（iteration 110 已实现：`TokenSetAuthProvider`、`useTokenSetAuthRegistry` / `useTokenSetAuthService` / `useTokenSetAuthState` / `useTokenSetAccessToken` / `useTokenSetCallbackResume`、`TokenSetCallbackComponent`（保留 `TokenSetCallbackOutlet` 兼容别名）；React Query subpath `/react-query` 中的 `useTokenSetReadinessQuery`；review-1 跟进后 callback path 在调用 `handleCallback()` 前会 await `registry.whenReady(clientKey)`，并暴露 `CallbackResumeStatus = "idle" | "pending" | "resolved" | "error"`，证据见 `examples/react-callback-async-readiness.test.ts`）
      ...
      - ~~`apps/webui` reference-app mutation dogfooding~~（iteration 114 已实现：create-group 写路径在 reference app 中证明了真实 mutation lifecycle 与 invalidation 语义，为 iteration 115 将其提升为 SDK owner 打下了基础）
      - ~~`apps/webui` React Query canonical 写路径采用~~（iteration 115 已实现：`@securitydept/token-set-context-client-react/react-query` 现已拥有 canonical groups / entries mutation hooks、token-set management entity contract 以及 query-key / invalidation policy；`apps/webui` dashboard 与 token-set reference page 都已直接消费 SDK-owned mutation hooks，app-local `useTokenSetQueries.ts` owner layer 已移除）
      - ~~`apps/webui` React canonical keyed consumer-path 收口~~（iteration 116 已实现：`@securitydept/token-set-context-client-react` 新增 `useTokenSetBackendOidcClient(clientKey)` 作为 SDK-owned lower-level accessor，`./react-query` canonical hooks 改为按 key 自解析 client、不再要求显式 `client`，而 `apps/webui` 的 login / dashboard / token-set page 主路径已不再依赖 app-local typed getter 或 `service.client as ...` narrowing）
      - ~~`apps/webui` multi-context auth shell 收口~~（iteration 114 review 后续已实现：`/login` 现为稳定 chooser；Token Set backend-mode 登录已成为真实 `/auth/token-set/backend-mode/login` OIDC 入口；backend-mode reference page 已迁到 `/playground/token-set/backend-mode`；session callback 显式对齐到 `/auth/session/callback`；dashboard 的 route gating、current-user/logout、groups/entries CRUD 均按持久化的 `AuthContextMode` 分流）
      - ~~`apps/webui` / `apps/server` token-set reference path 拆成 backend mode 与 frontend mode~~（iteration 117 已实现：旧的 generic token-set 宿主路径现已明确收成 backend mode；宿主同时提供 `/api/auth/token-set/frontend-mode/config` 作为 frontend-mode config projection；`/auth/token-set/frontend-mode/callback` 现在由 `TokenSetCallbackComponent` 真正承接；dashboard bearer integration 与 TanStack route security 已在两条 token-set mode 上被真实验证，且没有新增独立 React secure-guard layer）

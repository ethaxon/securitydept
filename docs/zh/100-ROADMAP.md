...
本文是 SecurityDept 当前 planning authority，负责描述当前 `0.3.x` release line、active release constraints，以及延期到 `0.3.x` 之后的主题。
...
当前 release baseline 是 `0.3.x` 线。

详细的 release execution record 现在放在 CHANGELOG.md 和 [008-RELEASE_AUTOMATION.md](008-RELEASE_AUTOMATION.md)。这个 roadmap 只保留 active release constraints，以及未来延期主题。
...
本地测试基础设施文档放在它验证的能力文档中：real-IP Docker/Kubernetes provider test behavior 记录在 `006`，release workflow behavior 继续记录在 `008`。

...
Structured diagnosis 是与 public error presentation 分离的另一层契约。面向 operator 的、secret-safe machine-readable auth-flow diagnosis 属于 `securitydept-utils::observability`，应独立保留，而不是被折叠成 adopter-facing error copy。
...
- Structured diagnosis fields 应保持 machine-readable 且 secret-safe。它们用于日志与 operator tooling，不应被当作生成 public browser/API copy 的捷径。

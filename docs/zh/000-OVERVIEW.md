...
- `securitydept-auth-runtime`
  - 面向路由的认证编排，并按 basic-auth、session 与 token-set 三类能力独立 feature 控制
- `securitydept-basic-auth-context`
  - 可复用的 basic-auth zone 与 redirect 抽象，并提供与框架无关的响应元数据
- `securitydept-session-context`
  - 可复用的 session 上下文抽象，供 cookie-session 模式使用，且不再直接耦合 Axum
- `securitydept-token-set-context`
  - 可复用的认证状态、redirect、metadata redemption 与 token 传播层，供无状态 token-set 模式使用

...
- **CI/CD**: GitHub Actions workflow `.github/workflows/docker-build.yml` builds and publishes `linux/amd64` images to GHCR on `v*` tags with SemVer aliases.
...
3. **Release quality**: Add container smoke tests, re-introduce native `arm64` build lane, and optionally signed images.

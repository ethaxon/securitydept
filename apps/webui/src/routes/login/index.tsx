...
import { AuthContextMode, type AuthLoginOptions } from "@/auth/model";
...
export const Route = createFileRoute("/login/")({
	...
	validateSearch(search): { post_auth_redirect_uri?: string } {
		return typeof search.post_auth_redirect_uri === "string"
			? { post_auth_redirect_uri: search.post_auth_redirect_uri }
			: {};
	},
});
...
function RouteComponent() {
	...
	const login = useMutation({
		mutationFn: (options: AuthLoginOptions) => authService.login(options),
	});
				...
				<div className="w-full max-w-md space-y-6 rounded-xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
					...
					<div className="space-y-3">
						...
						<a
							...
							href={TOKEN_SET_BACKEND_MODE_CONFIG.paths.playground}
						...
						<a
							...
							href={TOKEN_SET_FRONTEND_MODE_CONFIG.paths.playground}

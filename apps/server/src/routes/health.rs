...
pub async fn health(Query(query): Query<HealthQuery>) -> Json<HealthResponse> {
    let apis = if query.api_details {
        Some(vec![
            ...
            ApiRouteInfo {
                method: "GET",
                path: "/auth/token-set/login",
                auth_required: false,
                description: "Start stateless token-set OIDC login flow",
            },
            ApiRouteInfo {
                method: "GET",
                path: "/auth/token-set/callback",
                auth_required: false,
                description: "OIDC callback endpoint for token-set mode",
            },
            ApiRouteInfo {
                method: "POST",
                path: "/auth/token-set/refresh",
                auth_required: false,
                description: "Refresh stateless token-set credentials",
            },
            ApiRouteInfo {
                method: "POST",
                path: "/auth/token-set/metadata/redeem",
                auth_required: false,
                description: "Redeem token-set metadata by redemption id",
            },
            ApiRouteInfo {
                method: "GET",
                path: "/api/entries",

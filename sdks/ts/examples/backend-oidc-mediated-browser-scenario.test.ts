...
describe("external OIDC-mediated browser scenario", () => {
	it("supports bootstrap, callback handling, refresh, and disposal from browser-facing entry points", async () => {
		...
		const emptyBootstrap = await bootstrapBackendOidcMediatedModeClient(
			client,
			{
				location: {
		...
		const callbackBootstrap = await bootstrapBackendOidcMediatedModeClient(
			client,
			{
				location: {
					href: "https://app.example.com/oidc-mediated?tab=demo#access_token=callback-at&refresh_token=callback-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-1",

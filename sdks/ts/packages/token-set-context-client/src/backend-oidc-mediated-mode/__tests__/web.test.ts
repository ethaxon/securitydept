...
import { createBackendOidcMediatedModeAuthorizedTransport } from "../auth-transport";
import {
...
describe("token-set web helpers", () => {
	it("captures callback fragments and clears only the URL hash from history", async () => {
		...
		const callbackFragmentStore =
			createBackendOidcMediatedModeCallbackFragmentStore(sessionStore);
		const history = createHistoryRecorder();

		const fragment =
			await captureBackendOidcMediatedModeCallbackFragmentFromUrl({
				location: {
					href: "https://app.example.com/oidc-mediated?tab=demo#access_token=callback-at",
	...
	it("bootstraps browser client state from a callback fragment", async () => {
		...
		const result = await bootstrapBackendOidcMediatedModeClient(client, {
			location: {
				href: "https://app.example.com/oidc-mediated#access_token=callback-at&refresh_token=callback-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-1",
		...
		expect(result.source).toBe(BackendOidcMediatedModeBootstrapSource.Callback);
	...
	it("retains callback fragments when bootstrap fails with a retryable error", async () => {
		...
		await expect(
			bootstrapBackendOidcMediatedModeClient(client, {
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=callback-at&metadata_redemption_id=meta-1",
	...
	it("resets browser state by clearing both callback fragments and persisted auth", async () => {
		...
		await resetBackendOidcMediatedModeBrowserState(
			client,
			callbackFragmentStore,
		);
		...
		const restoredClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport: {
	...
	it("restores persisted auth across fresh browser clients sharing the same stores", async () => {
		...
		const callbackFragmentStore =
			createBackendOidcMediatedModeCallbackFragmentStore(sessionStore);
		const firstClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		const callbackResult = await bootstrapBackendOidcMediatedModeClient(
			firstClient,
			{
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=callback-at&refresh_token=callback-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-1",
		...
		expect(callbackResult.source).toBe(
			BackendOidcMediatedModeBootstrapSource.Callback,
		);
		expect(await callbackFragmentStore.load()).toBeNull();
		...
		const restoredResult = await bootstrapBackendOidcMediatedModeClient(
			restoredClient,
			{
				location: {
		...
		expect(restoredResult.source).toBe(
			BackendOidcMediatedModeBootstrapSource.Restore,
		);
		...
		const freshClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		const emptyResult = await bootstrapBackendOidcMediatedModeClient(
			freshClient,
			{
				location: {
					href: "https://app.example.com/oidc-mediated",
	...
	it("prefers a pending callback fragment over persisted auth and replaces the old state", async () => {
		...
		const callbackFragmentStore =
			createBackendOidcMediatedModeCallbackFragmentStore(sessionStore);
		const oldClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		const bootstrapClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		const callbackResult = await bootstrapBackendOidcMediatedModeClient(
			...
			{
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=new-at&refresh_token=new-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-new",
	...
	it("keeps callback retry precedence over persisted auth until a later retry succeeds", async () => {
		...
		const retryingClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		await expect(
			bootstrapBackendOidcMediatedModeClient(retryingClient, {
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=new-at&refresh_token=new-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-new",
					hash: "#access_token=new-at&refresh_token=new-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-new",
				},
		...
		const recoveredResult = await bootstrapBackendOidcMediatedModeClient(
			retryingClient,
			{
				location: {
					href: "https://app.example.com/oidc-mediated",
	...
	it("clears non-retryable callback precedence and only restores old state on a later fresh bootstrap", async () => {
		...
		const failingClient = createBackendOidcMediatedModeBrowserClient({
			baseUrl: "https://auth.example.com",
			persistentStore,
			sessionStore,
			transport,
		...
		await expect(
			bootstrapBackendOidcMediatedModeClient(failingClient, {
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=bad-at&refresh_token=bad-rt&metadata_redemption_id=meta-bad",
	...
	it("replaces a retained pending fragment with the latest URL callback before bootstrap", async () => {
		...
		const capturedFragment =
			await captureBackendOidcMediatedModeCallbackFragmentFromUrl({
				location: {
					href: "https://app.example.com/oidc-mediated?tab=members#access_token=new-at&refresh_token=new-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-new",
					hash: "#access_token=new-at&refresh_token=new-rt&expires_at=2026-01-01T00%3A05%3A00Z&metadata_redemption_id=meta-new",
				},
				history,
				callbackFragmentStore,
	...
	it("replaces a retry-retained pending fragment with the latest URL callback before recovery bootstrap", async () => {
		...
		await expect(
			bootstrapBackendOidcMediatedModeClient(retryingClient, {
				location: {
					href: "https://app.example.com/oidc-mediated#access_token=retry-at&refresh_token=retry-rt&metadata_redemption_id=meta-retry",
	...
	it("injects the current bearer and forwards cancellation tokens", async () => {
		...
		const transport = createBackendOidcMediatedModeAuthorizedTransport(
			{

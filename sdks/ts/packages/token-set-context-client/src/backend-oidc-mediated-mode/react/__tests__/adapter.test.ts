...
import type { BackendOidcMediatedModeClient } from "../../client";
import {
	BackendOidcMediatedModeContextProvider,
	type BackendOidcMediatedModeContextProviderProps,
	useAccessToken,
...
describe("token-set react adapter", () => {
	...
	it("syncs signal updates into React and disposes the client on unmount", async () => {
		...
		let exposedClient: BackendOidcMediatedModeClient | null = null;

		function Probe() {
			const { client } = useBackendOidcMediatedModeContext();
		...
		const providerProps: BackendOidcMediatedModeContextProviderProps = {
		...
		const view = render(
			createElement(BackendOidcMediatedModeContextProvider, providerProps),
		);
		...
		const client: BackendOidcMediatedModeClient = exposedClient;
	...
	it("disposes the old client and isolates subscriptions after provider reconfigure", async () => {
		...
		const observedTokens: Array<string | null> = [];
		const exposedClients: BackendOidcMediatedModeClient[] = [];
		...
		const firstProps: BackendOidcMediatedModeContextProviderProps = {
		...
		const view = render(
			createElement(BackendOidcMediatedModeContextProvider, firstProps),
		);
		...
		const secondProps: BackendOidcMediatedModeContextProviderProps = {
			...firstProps,
		...
		view.rerender(
			createElement(BackendOidcMediatedModeContextProvider, secondProps),
		);
		await flushMicrotasks();
	...
	it("keeps the current client alive across StrictMode remounts without stale subscriptions", async () => {
		...
		let activeClient: BackendOidcMediatedModeClient | null = null;

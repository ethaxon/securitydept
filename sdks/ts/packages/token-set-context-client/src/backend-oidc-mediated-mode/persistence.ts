...
// - The OIDC-mediated sealed flow uses the same AuthSnapshot shape, so the generic
//   persistence can handle the actual store/parse/validate work.
// - The token-set specific error codes (token_set.*) and the
//   BackendOidcMediatedModeContextSource.Persistence trace source need to remain
...
export interface BackendOidcMediatedModeStatePersistence {
 ...
 */
export function createBackendOidcMediatedModeStatePersistence(options: {
	store: RecordStore;
	key: string;
	now: () => number;
}): BackendOidcMediatedModeStatePersistence {
 ...
 * the OIDC-mediated v1 surface has historically advertised `token_set.persistence.*`
...
function remapPersistenceError(cause: unknown): unknown {
	...
	return new ClientError({
		...
		source: BackendOidcMediatedModeContextSource.Persistence,
		cause,
	});
}

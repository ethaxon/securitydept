...
// adds only:
//   - A unified OidcClientConfig vocabulary (vs scattered oauth4webapi params)
...
// ---------------------------------------------------------------------------
// OidcClient — thin wrapper around oauth4webapi protocol steps
// ---------------------------------------------------------------------------

/** An OIDC client instance bound to a specific provider + client config. */
export interface OidcClient {
	/** The resolved configuration. */
	readonly config: Readonly<OidcClientConfig>;
 ...
 *
 * const oidc = createOidcClient({ issuer: "https://auth.example.com", clientId: "spa", redirectUri: "https://app.example.com/callback" });
 * await oidc.discover();
 *
...
export function createOidcClient(config: OidcClientConfig): OidcClient {
	...
	return {
		...
		async authorize(params?: AuthorizeParams): Promise<AuthorizeResult> {
			if (!authServer) {
				throw new Error("OidcClient: call discover() before authorize()");
			}
			if (!authServer.authorization_endpoint) {
				throw new Error(
					"OidcClient: authorization_endpoint not found in discovery",
				);
			}
		...
		async handleCallback(
			...
			if (!authServer) {
				throw new Error("OidcClient: call discover() before handleCallback()");
			}

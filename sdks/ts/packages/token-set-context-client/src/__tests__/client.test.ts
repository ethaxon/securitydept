...
describe("TokenSetContextClient", () => {
	afterEach(() => {
		vi.unstubAllGlobals();
	});
	...
	it("refreshes tokens from a redirect fragment", async () => {
		...
		const client = new TokenSetContextClient({ baseUrl: BASE_URL }, runtime);
	...
	it("persists callback state, supports explicit restore, and only clears on explicit clear", async () => {
		...
		const restoredClient = new TokenSetContextClient(
			{ baseUrl: BASE_URL },
			restoredRuntime,
		);
	...
	it("emits trace events for restore, scheduled refresh, and refresh completion", async () => {
		...
		const client = new TokenSetContextClient(
			{
				baseUrl: BASE_URL,

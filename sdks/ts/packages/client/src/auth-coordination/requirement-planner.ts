...
//
// Core concept:
//   1. Define a list of AuthRequirements (e.g. "need session", "need oidc token")
...
//
// Owner rationale: This module does not depend on token-set domain objects
// (token snapshots, bearer propagation, OIDC callback, refresh). Its
// RequirementKind vocabulary covers session, OIDC, and custom — clearly
// cross-auth-context. It is a shared coordination capability, not a
// token-set product surface. Canonical owner is @securitydept/client.
...
export interface AuthRequirement {
	 ...
	 *
	 * An opaque string. Each auth-context family (token-set, session, basic-auth)
	 * or adopter project may define its own named constants.
	 *
	 * @example "session", "frontend_oidc", "backend_oidc", "custom"
	 */
	kind: string;

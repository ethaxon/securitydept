...
function parseBase64UrlJsonObject(value: string, label: string): JwtClaimsSet {
	...
	try {
		...
		throw new Error(`${label} must be base64url-encoded JSON`, { cause });
	}
...
function validateRegisteredClaimTypes(
	...
	if (claims.exp !== undefined && typeof claims.exp !== "number") {
		throw new Error(`${label} "exp" claim must be a number`);
	}
	if (claims.iat !== undefined && typeof claims.iat !== "number") {
		throw new Error(`${label} "iat" claim must be a number`);
	}
	if (claims.nbf !== undefined && typeof claims.nbf !== "number") {
		throw new Error(`${label} "nbf" claim must be a number`);
	}
	if (claims.iss !== undefined && typeof claims.iss !== "string") {
		throw new Error(`${label} "iss" claim must be a string`);
	}
	if (
		...
		throw new Error(`${label} "aud" claim must be a string or string array`);
	}
}
